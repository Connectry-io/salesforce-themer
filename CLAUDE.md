...
After editing `themes/engine.js` or `themes/themes.json`, **always run `node scripts/render-presets.mjs`** before committing. Every preset's CSS + favicon at `content/themes/*.css` + `content/favicons/*.svg` are AUTOGENERATED from the canonical engine; CI (`.github/workflows/render-presets.yml`) fails the build if they drift. Follow up with `node scripts/check-theme-drift.js` + `node scripts/check-effects-drift.js` — both must exit 0 locally.

(Legacy: `python3 scripts/sync-engine.py` was the pre-A2 flow that inlined the engine into `background.js`. That path is gone — engine is Studio-side only now.)
