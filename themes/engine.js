...
/* Icon background defaults — clear --slds-c-icon-color-background ONLY on
 * search/input/lookup contexts where SF would otherwise paint a stray
 * accent bubble behind the icon. CRITICAL: do NOT clear this at :root —
 * standard object icons (.slds-icon-standard-task, -event, -call, -email,
 * etc.) use this token (with their own colored fallback like
 *   var(--slds-c-icon-color-background, rgb(59, 167, 85))
 * ) to render the colored square behind the white symbol in places like the
 * activity composer. A :root override turns the green/blue/orange squares
 * transparent, leaving only the white inverse symbol invisible on the
 * neutral button. Scope tightly. */
.slds-input__icon,
.slds-input-has-icon,
.slds-input-has-icon .slds-icon,
.slds-input-has-icon-group,
.slds-form-element__icon,
.forceSearchInputContainer,
.forceSearchInputContainer .slds-icon,
.globalSearchContainer,
.globalSearchContainer .slds-icon,
.slds-lookup__menu,
.slds-lookup__menu .slds-icon {
