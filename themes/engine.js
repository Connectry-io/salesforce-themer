...
/* Icon background defaults — clear --slds-c-icon-color-background globally
 * to prevent stray accent bubbles behind generic icons (search magnifying
 * glass, global header buttons, setup gear, etc.).
 *
 * BUT: standard / custom / action icon containers (.slds-icon-standard-task,
 * .slds-icon-action-call, .slds-icon-custom-*) WANT a colored square
 * background — that's the whole point of those icon families (green for task,
 * blue for call, etc.). Their inner span uses a CSS variable with an inline
 * color fallback:
 *   style="--sds-c-icon-color-background: var(--slds-c-icon-color-background, rgb(59, 167, 85))"
 * The fallback only kicks in when --slds-c-icon-color-background is INVALID
 * (not when it's set to "transparent"). Setting it to the initial value on
 * those specific containers makes var() see it as guaranteed-invalid → uses the
 * inline fallback color. Best of both worlds: generic icons stay clean,
 * standard/custom/action icons keep their SF brand colors. */
:root {
...
[class*="slds-icon-action-"] {
  --slds-c-icon-color-background: initial !important;
  --sds-c-icon-color-background: initial !important;
}

/* Same scoping note as above: do NOT include bare .slds-icon_container —
 * that class lives on standard object icons whose colored square background
 * IS painted via background-color (sourced from --sds-c-icon-color-background).
 * Force only the search/input icon contexts to transparent. Standard object
 * icons get to keep their SF brand colors (green task, blue call, etc.). */
.slds-input__icon,
.slds-input-has-icon .slds-icon,
.forceSearchInputContainer .slds-icon_container,
.globalSearchContainer .slds-icon_container,
.slds-form-element__icon .slds-icon_container,
.slds-input-has-icon .slds-icon_container {
  background-color: transparent !important;
}
 ...
 * in a standard .slds-modal__container. Native SF renders this as a
 * centered card with the Setup page dimmed behind via backdrop; we
 * match that pattern. Prior fixes (B8 transparent, B25 fullscreen)
 * were both wrong — the card should be a proper themed dark surface
 * sized to content, letting the backdrop handle the page dim. */
...
/* B23 — App Launcher deep fix. DOM capture 2026-04-18 revealed the real
 * class names (SF uses .al-* internally + Shadow DOM parts for rich text).
 * Previous batches missed these because they're not standard SLDS names. */

/* .al-modal-* is the real App Launcher modal chrome */
one-app-launcher-modal .al-modal-container,
one-app-launcher-modal .al-modal-body,
one-app-launcher-modal .al-modal-list {
  background-color: ${c.modalBg} !important;
  color: ${c.textPrimary} !important;
}

one-app-launcher-modal .al-modal-header {
  background-color: ${c.modalHeaderBg} !important;
  color: ${c.textPrimary} !important;
  border-color: ${c.border} !important;
}

/* Tile description ("The Sales application." etc.) — real class is
 * .al-app-tile-description, not .slds-text-body_small */
one-app-launcher-app-tile .al-app-tile-description,
body .al-app-tile-description {
  color: ${c.textSecondary} !important;
}

/* Tile title renders inside lightning-formatted-rich-text with Shadow DOM.
 * The content is in part="formatted-rich-text" (piercing via ::part) plus
 * the legacy .slds-rich-text-editor__output wrapper. Both need the accent
 * color so the app name reads as a link. */
one-app-launcher-app-tile lightning-formatted-rich-text,
one-app-launcher-app-tile .slds-rich-text-editor__output,
one-app-launcher-app-tile .slds-rich-text-editor__output p,
one-app-launcher-app-tile .slds-rich-text-editor__output span {
  color: ${c.accent} !important;
  font-weight: 600 !important;
  background-color: transparent !important;
}

one-app-launcher-app-tile lightning-formatted-rich-text::part(formatted-rich-text) {
  color: ${c.accent} !important;
}

/* Tab row (All Apps / All Items) — .al-tab-item + .label-display */
one-app-launcher-tab-item,
one-app-launcher-tab-item .al-tab-item,
one-app-launcher-tab-item .label-display,
body .al-tab-item,
body .al-tab-item .label-display {
  color: ${c.textPrimary} !important;
  background-color: transparent !important;
}

one-app-launcher-tab-item .al-tab-item[aria-selected="true"],
one-app-launcher-tab-item .al-tab-item.slds-is-active,
body .al-tab-item[aria-selected="true"] {
  color: ${c.accent} !important;
  border-bottom-color: ${c.accent} !important;
}

/* Tile avatar wrapper — lightning-avatar contains the img, make sure
 * the container doesn't force a white bg behind the icon */
one-app-launcher-app-tile lightning-avatar .slds-avatar,
one-app-launcher-app-tile lightning-avatar .slds-avatar_large {
  background-color: transparent !important;
}
