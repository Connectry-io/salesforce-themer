...
(() => {
  ...
  // Tiered region sampler (BACKLOG §1k).
  //
  // Tier priority: CONTENT > MAIN > CHROME > FALLBACK.
  // We force at least one anchor from CONTENT/MAIN when available so the
  // header doesn't monopolize gap examples. Fills remaining slots with
  // other tiers, deduped by element.
  //
  // Tiers:
  //   chrome  — global chrome (header, nav rails, footers). High recall,
  //             low diversity — every page has these.
  //   main    — big content regions: <main>, role=main, Setup-specific
  //             wrappers, .slds-page-main. Contains the "interesting"
  //             page body.
  //   content — specific high-information elements INSIDE main: tables,
  //             forms, card bodies, listview grids, record body. These
  //             carry the token families most likely to surface gaps.
  const REGION_TIERS = {
    content: [
      'table.slds-table, .slds-table tbody tr',
      '.slds-form, form.slds-form_stacked, form.slds-form--stacked, .slds-form-element',
      '.slds-card__body, article.slds-card .slds-card__body',
      '.forceListViewManagerGrid, .slds-table_resizable-cols',
      '.slds-form__section, .slds-section__content',
      'records-record-layout-row, records-record-layout-block',
    ],
    main: [
      'main, [role="main"]',
      '.forceSetupAppContent, .setupContent, .setup-main-content',
      '.slds-page-main, .slds-template_default',
      '.forceRelatedListContainer, .forceDetailPanel',
    ],
    chrome: [
      'header, #oneHeader, .slds-global-header_container',
      'nav, [role="navigation"], .setup-nav, .slds-vertical-tabs__nav, .slds-nav-vertical',
      'aside, [role="complementary"]',
      'footer, [role="contentinfo"]',
    ],
  };

  function firstVisibleInRegion(regionEl) {
    if (!regionEl) return null;
    const all = regionEl.querySelectorAll('[class*="slds-"], [class*="lwc-"]');
    for (const el of all) {
      if (el === regionEl) continue;
  ...
  // Pick first anchor from any selector in a tier. Returns null if no
  // selector matches a visible element.
  function firstAnchorInTier(selectors, seen) {
    for (const sel of selectors) {
      const regions = document.querySelectorAll(sel);
      for (const region of regions) {
        if (isEffectivelyHidden(region)) continue;
        const anchor = firstVisibleInRegion(region) || (isEffectivelyHidden(region) ? null : region);
        if (anchor && !seen.has(anchor)) return anchor;
      }
    }
    return null;
  }
  ...
  function findElementsForToken(token, max = 3) {
    const picked = [];
    const seen = new Set();
    // Tier priority: content first (most informative), then main, then chrome.
    for (const tier of ['content', 'main', 'chrome']) {
      if (picked.length >= max) break;
      const anchor = firstAnchorInTier(REGION_TIERS[tier], seen);
      if (anchor) {
        seen.add(anchor);
        picked.push(anchor);
      }
    }
    // Fill remaining slots: try a second anchor from content/main to diversify.
    for (const tier of ['content', 'main']) {
      if (picked.length >= max) break;
      const anchor = firstAnchorInTier(REGION_TIERS[tier], seen);
      if (anchor) {
        seen.add(anchor);
    ...
    // Final fallback: global pool so we never return 0 elements.
