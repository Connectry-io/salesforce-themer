...
(() => {
  ...
  function sanitizeNode(el) {
    ...
    for (const a of el.attributes || []) {
      ...
      // Strip anything that looks like content or PII
      if (a.name === 'value' || a.name === 'placeholder' || a.name === 'aria-label') {
        out.attrs[a.name] = `[redacted len=${(a.value || '').length}]`;
        continue;
      }
      if (a.name.startsWith('data-') || a.name.startsWith('aria-') || a.name === 'role' || a.name === 'id') {
        out.attrs[a.name] = clip(String(a.value || ''), 200);
      }
    }
    return out;
  }
  ...
  function findElementsForToken(token, max = 3) {
    ...
    for (const el of all) {
      ...
      const cs = getComputedStyle(el);
      const probes = [
        cs.color, cs.backgroundColor, cs.borderColor, cs.borderTopColor,
        cs.outlineColor, cs.fill, cs.stroke,
      ];
      // The actual computed value won't contain the var() — but the *original*
      // declaration may. Use cssText match as a heuristic.
      try {
        const sheets = el.matches(':hover') ? [] : [];
        // Simpler: just take the first few SLDS containers as anchors.
        candidates.push(el);
      } catch { /* */ }
    }
