...
(() => {
  ...
  // Sample anchor elements from distinct page regions so we don't
  // monopolize on whichever region appears first in DOM order (usually
  // the global header). Without this, Setup scans produce header-only
  // gaps forever, never sampling main content (BACKLOG §1k, 2026-04-15).
  const REGION_SELECTORS = [
    { name: 'header', sel: 'header, #oneHeader, .slds-global-header_container' },
    { name: 'nav',    sel: 'nav, [role="navigation"], .setup-nav, .slds-vertical-tabs__nav, .slds-nav-vertical' },
    { name: 'main',   sel: 'main, [role="main"], .forceSetupAppContent, .setup-main-content, .slds-page-main, .slds-template_default' },
    { name: 'aside',  sel: 'aside, [role="complementary"], .slds-docked_bottom-right, .slds-docked-composer-container' },
  ...
  function findElementsForToken(token, max = 3) {
    const picked = [];
    const seen = new Set();
    // Round 1: one anchor per detected region.
    for (const { sel } of REGION_SELECTORS) {
      if (picked.length >= max) break;
      const region = document.querySelector(sel);
      if (!region || isEffectivelyHidden(region)) continue;
      const anchor = firstVisibleInRegion(region);
      if (anchor && !seen.has(anchor)) {
        seen.add(anchor);
        picked.push(anchor);
      }
    }
    // Round 2: fill remaining slots from the global pool.
