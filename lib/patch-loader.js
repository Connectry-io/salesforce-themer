...
(() => {
  ...
  async function fetchIndex() {
    // SECURITY (T4): default tier = global. Only opt-in QA mode loads
    // internal-tier patches. Customer machines never see qa rows.
    let tierParam = 'global';
    try {
      if (chrome?.storage?.local) {
        const { sfThemerQAMode } = await chrome.storage.local.get('sfThemerQAMode');
        if (sfThemerQAMode === true) tierParam = 'global,qa';
      }
    } catch {}
