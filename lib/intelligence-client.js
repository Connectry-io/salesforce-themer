...
(() => {
  ...
  async function decideSuggestion(id, { decision, rejectReason = null, publish = false, tier = 'draft' }) {
    const anonId = await getAnonUserId();
    const headers = {
      'content-type': 'application/json',
      'x-connectry-secret': DEV_SHARED_SECRET,
    };
    // Publishing to 'published' tier requires the publish secret (HQ-only).
    if (publish && tier === 'published') {
      const ps = await getPublishSecret();
      if (!ps) {
        return {
          error: 'publish-secret-missing',
          message: 'Publishing to "published" tier requires the Connectry publish secret. Set it in chrome.storage.local.sfThemerPublishSecret on the HQ machine.',
        };
      }
