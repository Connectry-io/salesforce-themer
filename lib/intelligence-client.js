...
(() => {
  ...
  async function decideSuggestion(id, { decision, rejectReason = null, publish = false, tier = 'qa' }) {
    const anonId = await getAnonUserId();
    const headers = {
      'content-type': 'application/json',
      'x-connectry-secret': DEV_SHARED_SECRET,
    };
    // Publishing to 'global' requires the publish secret (HQ-only).
    if (publish && tier === 'global') {
      const ps = await getPublishSecret();
      if (!ps) {
        return {
          error: 'publish-secret-missing',
          message: 'Publishing to global tier requires the Connectry publish secret. Set it in chrome.storage.local.sfThemerPublishSecret on the HQ machine.',
        };
      }
