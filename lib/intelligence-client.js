...
(() => {
  ...
  // ── V1 lite telemetry — fire-and-forget anonymous aggregate counters ────
  // These are NOT consent-gated because they're truly anonymous aggregate
  // stats (no PII, no SF hostnames, no DOM data, no custom-theme content).
  // Privacy policy declares this collection clearly. The bigger
  // sendEvent() above stays consent-gated for AI/scan payloads which DO
  // include richer data.
  //
  // Use sparingly — count things, not journeys. If you find yourself
  // reaching for trackDaily on every click, you want sendEvent + consent
  // instead.

  // Detect whether this install came from the Web Store (packed) or is
  // an unpacked dev load. Trick: store installs always have update_url
  // in the runtime manifest; unpacked installs never do. Zero permissions
  // required, no Chrome API calls. Auto-tagged onto every event so
  // dev/QA traffic never conflates with real beta-tester signals — just
  // filter install_type='store' in your dashboards.
  function _installType() {
    try {
      const m = chrome?.runtime?.getManifest?.();
      return m && m.update_url ? 'store' : 'dev';
    } catch (_) { return 'dev'; }
  }
  ...
  function track(eventName, payload = {}) {
    try {
      // Sanitize: only allow primitive values + small whitelisted keys.
      // Prevents accidental PII leakage if a caller passes the wrong thing.
      const safePayload = {
      ...
      for (const k of Object.keys(payload)) {
        ...
        if (v == null) continue;
        const t = typeof v;
        if (t === 'string' && v.length <= 64) safePayload[k] = v;
        else if (t === 'number' || t === 'boolean') safePayload[k] = v;
      }
      // Fire and forget — never block the caller, never throw.
      sendEvent(eventName, safePayload, { requireConsent: false }).catch(() => {});
    } catch (_) {
      // swallow — telemetry must never break the product
    }
  }

  /**
   * Daily-debounced track. Use for "is this user active today" signals
   * that should fire at most once per UTC day per install. Stored in
   * chrome.storage.local so it survives reloads.
   */
  async function trackDaily(eventName, payload = {}) {
    try {
      const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD UTC
      const storageKey = `sftDailyTrack_${eventName}`;
      const obj = await chrome.storage.local.get(storageKey);
      if (obj?.[storageKey] === today) return; // already fired today
  ...
  // ── /themes — save a custom theme (Migration B, V2.5) ──────────────────
  // Writes both the `themes` row and the matching `app_configs` row at
  // key='themes/user/<slug>' atomically. Returns { slug, etag, version, key }
  // or { error }.
  async function saveTheme({ slug, owner, name, baseTokens, overrides, renderedCss }) {
    try {
      const res = await fetchWithTimeout(`${BASE_URL}/themes`, {
        method: 'POST',
        headers: {
          'content-type': 'application/json',
          'x-connectry-secret': DEV_SHARED_SECRET,
  ...
  // Delete a custom theme on the backend. Ownership enforced server-side.
  async function deleteTheme({ slug, owner }) {
    try {
      const res = await fetchWithTimeout(
        `${BASE_URL}/themes/${encodeURIComponent(slug)}?owner=${encodeURIComponent(owner)}`,
        {
          method: 'DELETE',
          headers: { 'x-connectry-secret': DEV_SHARED_SECRET },
        },
      );
      if (!res.ok) return { error: `HTTP ${res.status}: ${await res.text()}` };
      return await res.json();
  ...
  // List custom themes the backend has for this product. Used by the popup's
  // "My Themes" section to surface themes saved from other browsers/machines.
  // Returns { themes: [{ slug, name, owner, updated_at, rendered_css_etag }] }.
  async function listUserThemes() {
    try {
      const res = await fetchWithTimeout(`${BASE_URL}/themes`, { method: 'GET' });
      if (!res.ok) return { error: `HTTP ${res.status}` };
      return await res.json();
    } catch (err) {
      return { error: String(err) };
